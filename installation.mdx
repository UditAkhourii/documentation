---
title: "Installation"
description: "Every way to install ADHD — agent skill, CLI, library, Agent SDK, and per-platform paths."
icon: "download"
---
...
```bash Claude Code (global)
mkdir -p ~/.claude/skills/adhd
curl -fsSL https://raw.githubusercontent.com/UditAkhourii/adhd/main/skills/adhd/SKILL.md \
  -o ~/.claude/skills/adhd/SKILL.md
```

```bash Claude Code (per-project)
mkdir -p .claude/skills/adhd
curl -fsSL https://raw.githubusercontent.com/UditAkhourii/adhd/main/skills/adhd/SKILL.md \
  -o .claude/skills/adhd/SKILL.md
```

```bash Cursor (project rules)
